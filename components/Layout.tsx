...
const Layout: React.FC<LayoutProps> = ({ children, showAdminLink = true, isDarkMode, toggleDarkMode }) => {
  return (
    <div className="flex flex-col min-h-screen bg-white dark:bg-slate-950 theme-transition">
      <header className="glass border-b border-slate-200 dark:border-slate-800 sticky top-0 z-50 theme-transition">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
          <Link to="/" className="flex items-center gap-3 group">
            <div className="w-10 h-10 bg-primary-600 rounded-2xl flex items-center justify-center transform group-hover:rotate-12 transition-transform shadow-lg shadow-primary-500/20">
          ...
          <nav className="hidden md:flex items-center gap-10">
            <Link to="/" className="text-slate-600 dark:text-slate-400 hover:text-primary-600 dark:hover:text-primary-400 font-semibold transition">الرئيسية</Link>
            <Link to="/about" className="text-slate-600 dark:text-slate-400 hover:text-primary-600 dark:hover:text-primary-400 font-semibold transition">عن المشروع</Link>
            
            <div className="h-6 w-px bg-slate-200 dark:bg-slate-800 mx-2"></div>

            <button 
              onClick={toggleDarkMode}
              className="p-3 rounded-2xl bg-slate-100 dark:bg-slate-900 text-slate-600 dark:text-slate-400 border border-slate-200 dark:border-slate-800 hover:border-primary-400 hover:text-primary-600 transition-all shadow-sm flex items-center justify-center"
          ...
          <div className="md:hidden flex items-center gap-4">
            ...
            <Link to="/login" className="text-slate-500 dark:text-slate-400 p-2">
      ...
      <footer className="bg-slate-50 dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800 py-16 theme-transition">
        <div className="max-w-7xl mx-auto px-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-12 mb-12">
            <div>
              <Link to="/" className="flex items-center gap-3 mb-6">
                <div className="w-8 h-8 bg-primary-600 rounded-xl flex items-center justify-center shadow-md shadow-primary-500/10">
              ...
              <p className="text-slate-500 dark:text-slate-400 leading-relaxed max-w-sm">
                بوابتك نحو المعرفة الرقمية والابتكار المستقبلي. مقالات ملهمة وتقنيات حديثة في متناول يدك.
              </p>
            </div>
            <div>
              <h4 className="font-bold mb-6 text-slate-900 dark:text-white text-lg">روابط سريعة</h4>
            ...
            <div>
              <h4 className="font-bold mb-6 text-slate-900 dark:text-white text-lg">النشرة الإخبارية</h4>
              <p className="text-slate-500 dark:text-slate-400 mb-4 text-sm">اشترك لتصلك أحدث المقالات والتحليلات التقنية.</p>
              <div className="flex gap-2">
                <input type="email" placeholder="بريدك..." className="flex-grow bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 px-4 py-2 rounded-xl outline-none focus:ring-2 focus:ring-primary-500 dark:text-white transition" />
                <button className="bg-primary-600 text-white px-4 py-2 rounded-xl font-bold hover:bg-primary-700 transition">اشترك</button>
              </div>
            </div>
          </div>
          <div className="pt-8 border-t border-slate-200 dark:border-slate-800 text-center">
            <p className="text-slate-400 dark:text-slate-500 text-sm">© 2024 جميع الحقوق محفوظة لمدونة المستقبل</p>
          </div>
        </div>
      </footer>
    </div>
